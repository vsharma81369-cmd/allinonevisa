...
(function () {
  ...
  /* 14. Requirements checker (homepage hero + dedicated page) ------------
     Field-agnostic: it populates whichever of these exist in #reqForm:
     #hResident (country), #hPassport (country), #hDest1 + .dest-select
     (destination, optional #addDest), #hPurpose (optional). */
  function initRequirementsForm() {
    ...
    const residentSel = $('#hResident', form);
    const passportSel = $('#hPassport', form);
    const purposeSel = $('#hPurpose', form);
    const dest1 = $('#hDest1', form);

    fill(residentSel, countries, null, 'United States of America');
    fill(passportSel, countries, null, 'United States of America');
    fill(purposeSel, purposes, 'Select One');
    fill(dest1, destinations, 'Select Destination');

    // Optional: add more destinations (only if the controls exist)
    ...
    const clearErr = (sel) => { const f = sel && sel.closest('.field, .big-field'); if (f) f.classList.remove('has-error'); };
    form.addEventListener('change', (e) => { if (e.target.value) clearErr(e.target); });

    form.addEventListener('submit', (e) => {
      e.preventDefault();
      let ok = true, firstBad = null;
      const flag = (sel) => {
        const f = (sel.closest('.field, .big-field')) || sel;
        f.classList.add('has-error', 'shake');
        setTimeout(() => f.classList.remove('shake'), 400);
        ok = false; firstBad = firstBad || sel;
      };
      if (dest1 && !dest1.value) flag(dest1);
      if (purposeSel && !purposeSel.value) flag(purposeSel);
      if (!ok) { if (firstBad) firstBad.focus(); return; }

      const chosen = [];
      $$('.dest-select', form).forEach((s) => { if (s.value) chosen.push(s.value); });
      const type = purposeSel ? (purposeToType[purposeSel.value] || '') : '';
      try {
        localStorage.setItem('aiov_trip', JSON.stringify({
          resident: residentSel ? residentSel.value : '',
          passport: passportSel ? passportSel.value : '',
          destinations: chosen,
          purpose: purposeSel ? purposeSel.value : ''
        }));
        let app = {};
        try { app = JSON.parse(localStorage.getItem('aiov_application') || '{}'); } catch (e2) { app = {}; }
        if (chosen[0]) app.aDest = chosen[0];
        if (type) app.aType = type;
        if (passportSel) app.aNat = passportSel.value;
